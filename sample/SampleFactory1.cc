...
namespace NRS
{
  namespace Sample
  {
    /// Factory for Sample nodes, the root node for a sample component
    ...
    protected:
      ...
      {
	iDescription = "The root node of a sample component";
      }
	...
	anIDMap[ "out" ] = makeID( 0, -1, true );
        ...
        // triggered by MainLoop via the MessageFunctionObject
        // above. This alternates with the Updater variable to ensure
        // that all calculations are done before messages are sent out
        // so that there are no issues with the order in which
        // calculations are made.
	addVariable( "MessageSender", "", "MessageSender", false, false,
		     anIDMap, aSegAttrRefMap, aSegConsMap, aMFOGenMap );
	aMFOGenMap.clear();

        // MainLoop causes Updater to update and send all its messages
        aMFOGenMap["MainLoop"] = new Message::MainLoopMFOGenerator();

        // Signal to update state of variables in all nodes, triggered
        // by MainLoop via the MessageFunctionObject above. This
        // alternates with the MessageSender variable to ensure that
        // all calculations are done before messages are sent out so
        // that there are no issues with the order in which
        // calculations are made.
...
namespace
{
  NRS::Sample::SampleFactory sSampleF;
}
