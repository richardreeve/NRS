...
public class UniformCrossoverNode extends CrossoverNode
{
   ...
   */
  public UniformCrossoverNode(VariableManager vmMan, String vnName){
    super(vmMan, vnName, "UniformCrossoverNode");
  }
  
  //----------------------------------------------------------------------//
  /** {@link Message} to deliver to this Node. */
  public void deliver(Message m){
    PackageLogger.log.fine("Received message at UniformCrossoverNode!"); 
  ...
  public void crossover(ArrayList population, int index, 
    ...
    popSize = population.size(); //should be population size minus elite
    boolean[] crossed = new boolean[popSize];
    
    ArrayList child1, child2;
    rng = getRNGNode(); //save time not having to make call everytime
    Gene g, g1, temp;
    
    //Uniform crossover
    m_cross = new boolean[size];
    
    double limit = Math.floor(popSize/2);
    while ( counter <= limit ){
      ...
      child1 = (ArrayList) c.getGenes().clone(); //mum
      c = (Chromosome) population.get(index2);
      child2 = (ArrayList) c.getGenes().clone(); //dad
      
      //get random mask
      for(int i = 0; i < size; i++){
        m_cross[i] = (rng.getRandInt(2)==1) ? true : false;
      }
      ...
      if ( (rng.getRandInt(100)+1) <= getRate() ){
        ...
        for(int k = 0; k < m_cross.length-1; k+=2){
          if ( !m_cross[k] ) { //swap genes
            g = (Gene) child1.get(k);
            //make a copy of object to store in temp
            temp = (Gene) g.clone(); 
            g1 = (Gene) child2.get(k);
            g1 = (Gene) g1.clone();
            child1.set(k, g1); //set dad's side to be mum's genes
            child2.set(k, temp); //set mum's side to be dad's genes
          }
          //else children stay as they are
        }
      }
