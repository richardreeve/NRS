...
/** Class to represent BooleanValueNode
 * 
 * This class implements a subclass of ValueNode whose value is a boolean type.
 *
...
{
  //Variables
  private BooleanType m_varOutput, m_varInput, m_varValue;
   ...
   */
  protected BooleanValueNode(VariableManager vmMan, String vnName){
    super(vmMan, vnName, "BooleanValueNode");
    
    String varName = vnName+ ".Output";
    m_varOutput = new BooleanType(vmMan, varName, false, true){
    ...
    m_varInput = new BooleanType(vmMan, varName2, true, true){
        public void deliver(Message m)
        {
    ...
    m_varValue = new BooleanType(vmMan, varName3, true, true){
        public void deliver(Message m)
        {
          handleMessageAt_Value(m, this, checkType(m, this));
        }
        public void deliver(boolean b){
          handleMessageAt_Value(b);
        }
      };
  ...
  public void deliver(Message m){
    ...
    Boolean b = Boolean.valueOf(m.getField("value"));
    if ( b != null )
      m_varValue.setDefault(b.booleanValue());
  }
  //-----------------------------------------------------------------------//
  ...
  public void handleMessageAt_Trigger(Message m, Variable v, boolean diff){ 
	...
	m_varOutput.onEvent(m_varValue.getValue().booleanValue());
  }
   ...
   * @param b the received value
   */
  public void handleMessageAt_Value(boolean b) { 
    PackageLogger.log.fine("Received message at " + getVNName() 
                           + ":Value variable!");
  }
  //-----------------------------------------------------------------------//
  ...
  private void setConsistent(){
    ...
    if ( m_varValue.getValue().booleanValue() == 
         m_varInput.getValue().booleanValue() )
      b = true;
