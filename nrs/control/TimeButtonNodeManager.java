...
/** Manage TimeButtonNodes.
 *
 *
 * @author Theophile Gonos
*/

class TimeButtonNodeManager {
    ...
    /** Singleton instance of <code>TimeButtonNodeManager</code> class. */
    private static TimeButtonNodeManager m_singletonInstance = null;
    
    /** 
     * Return the singleton instance of class TimeButtonNodeManager.
     *
     * @return singleton instance of class TimeButtonNodeManager.
     */
    public static TimeButtonNodeManager getInstance(){
	if (m_singletonInstance == null) new TimeButtonNodeManager();
	
	return m_singletonInstance;
    }

    /** Private constructor, uses singleton patter. */
    private TimeButtonNodeManager(){
	...
	m_button_by_id = new HashMap();
     ...
     /** Add new TimeButtonNode by vnName (TimeButtonLabel variable) and vnID.
     *
     * @param bn JButton node to add 
    */
    public void add(TimeButtonNode bn){
    ...
    /** Delete TimeButton node by vnid.
     *
     * @param vnid id of TimeButton node to be removed.
    */
    public void remove(Integer vnid){
	String vnName = (String) m_name_by_id.remove(vnid);
	m_id_by_name.remove(vnName);

	TimeButtonNode bn = (TimeButtonNode) m_button_by_id.get(vnid);
	bn.removeFromGUI();
    }

    /** Delete TimeButton node by name.
     *
     * @param vnName name of TimeButton node to be removed.
    */
	...
	TimeButtonNode bn = (TimeButtonNode) m_button_by_id.get(id);
	bn.removeFromGUI();
    }

    /** Remove all TimeButtonNodes from TimeButtonNodeManager. */
    ...
    */
    ...
    /** Check that the vnID specified exists as a TimeButton node.
     *
     * @param vnid the VNID of variable or node to lookup.
     * @return determines whether vnID exists as a TimeButton node. Empty string if not.
     */
    public String checkTimeButtonType(Integer vnid){
	if ( m_button_by_id.containsKey(vnid) ) return "TimeButtonNode";
	else return "";
    }
