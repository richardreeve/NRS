...
class ButtonNodeManager {
    ...
    private static ButtonNodeManager m_singletonInstance = null;
     ...
     */
    public static ButtonNodeManager getInstance(){
	if (m_singletonInstance == null) new ButtonNodeManager();
	
	return m_singletonInstance;
    }

    /** Private constructor, uses singleton patter. */
    private ButtonNodeManager(){
	...
	m_button_by_id = new HashMap();
     ...
     /** Add new ButtonNode by vnName (ButtonLabel variable) and vnID.
     *
     * @param bn ButtonNode to add
    */
    public void add(ButtonNode bn){
    ...
    /** Delete Button node by vnid.
     *
     * @param vnid id of Button node to be removed.
    */
    public void remove(Integer vnid){
	String vnName = (String) m_name_by_id.remove(vnid);
	m_id_by_name.remove(vnName);

	ButtonNode bn = (ButtonNode) m_button_by_id.get(vnid);
	bn.removeFromGUI();
    }

    /** Delete Button node by name.
     *
     * @param vnName name of Button node to be removed.
    */
	...
	ButtonNode bn = (ButtonNode) m_button_by_id.get(id);
	bn.removeFromGUI();
    }

    /** Remove all ButtonNodes from ButtonNodeManager. */
    ...
    */
    ...
    /** Check that the vnID specified exists as a Button node.
     *
     * @param vnid the VNID of variable or node to lookup.
     * @return determines whether vnID exists as a Button node. Empty string if not.
     */
    public String checkButtonType(Integer vnid){
	if ( m_button_by_id.containsKey(vnid) ) return "ButtonNode";
	else return "";
    }
