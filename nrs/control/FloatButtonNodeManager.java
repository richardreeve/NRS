...
/** Manage FloatButtonNodes.
 *
 *
 * @author Thomas French
*/

class FloatButtonNodeManager {
    ...
    /** Singleton instance of <code>FloatButtonNodeManager</code> class. */
    private static FloatButtonNodeManager m_singletonInstance = null;
    
    /** 
     * Return the singleton instance of class FloatButtonNodeManager.
     *
     * @return singleton instance of class FloatButtonNodeManager.
     */
    public static FloatButtonNodeManager getInstance(){
	if (m_singletonInstance == null) new FloatButtonNodeManager();
	
	return m_singletonInstance;
    }

    /** Private constructor, uses singleton patter. */
    private FloatButtonNodeManager(){
	...
	m_button_by_id = new HashMap();
     ...
     /** Add new FloatButtonNode by vnName (FloatButtonLabel variable) and vnID.
     *
     * @param bn JButton node to add 
    */
    public void add(FloatButtonNode bn){
    ...
    /** Delete FloatButton node by vnid.
     *
     * @param vnid id of FloatButton node to be removed.
    */
    public void remove(Integer vnid){
	String vnName = (String) m_name_by_id.remove(vnid);
	m_id_by_name.remove(vnName);

	FloatButtonNode bn = (FloatButtonNode) m_button_by_id.get(vnid);
	bn.removeFromGUI();
    }

    /** Delete FloatButton node by name.
     *
     * @param vnName name of FloatButton node to be removed.
    */
	...
	FloatButtonNode bn = (FloatButtonNode) m_button_by_id.get(id);
	bn.removeFromGUI();
    }

    /** Remove all FloatButtonNodes from FloatButtonNodeManager. */
    ...
    */
    ...
    /** Check that the vnID specified exists as a FloatButton node.
     *
     * @param vnid the VNID of variable or node to lookup.
     * @return determines whether vnID exists as a FloatButton node. Empty string if not.
     */
    public String checkFloatButtonType(Integer vnid){
	if ( m_button_by_id.containsKey(vnid) ) return "FloatButtonNode";
	else return "";
    }
