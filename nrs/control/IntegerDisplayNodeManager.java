...
/** Manage IntegerDisplayNodes.
 *
 * @author Thomas French
*/

class IntegerDisplayNodeManager {
  ...
  /** Singleton instance of <code>IntegerDisplayNodeManager</code> class. */
  private static IntegerDisplayNodeManager m_singletonInstance = null;
  
  /** 
   * Return the singleton instance of class IntegerDisplayNodeManager.
   *
   * @return singleton instance of class IntegerDisplayNodeManager.
   */
  public static IntegerDisplayNodeManager getInstance(){
    if (m_singletonInstance == null) new IntegerDisplayNodeManager();
    
    return m_singletonInstance;
  }
  
  /** Private constructor, uses singleton patter. */
  private IntegerDisplayNodeManager(){
  ...
  /** Add new IntegerDisplayNode by vnName (IntegerDisplayLabel variable) 
   * and vnID.
   *
   * @param bn add this IntegerDisplayNode 
   */
  public void add(IntegerDisplayNode bn){
  ...
  /** Delete IntegerDisplay node by vnid.
   *
   * @param vnid id of IntegerDisplay node to be removed.
   */
  public void remove(Integer vnid){
    String vnName = (String) m_name_by_id.remove(vnid);
    m_id_by_name.remove(vnName);
    
    IntegerDisplayNode bn = (IntegerDisplayNode) m_button_by_id.get(vnid);
    bn.removeFromGUI();
  }
  
  /** Delete IntegerDisplay node by name.
   *
   * @param vnName name of IntegerDisplay node to be removed.
   */
  public void remove(String vnName){
    ...
    IntegerDisplayNode bn = (IntegerDisplayNode) m_button_by_id.get(id);
    bn.removeFromGUI();
  }
  
  /** Remove all IntegerDisplayNodes from IntegerDisplayNodeManager. */
  public void removeAll(){
    m_id_by_name.clear();
    m_name_by_id.clear();
    
    IntegerDisplayNode bn;
    //iterate through and remove all buttons from GUI
    for (Iterator i = m_button_by_id.values().iterator(); i.hasNext(); ){
      bn = (IntegerDisplayNode) i.next();
      bn.removeAll();
      bn.removeFromGUI();
    }
  }
  ...
  /** Check that the vnID specified exists as a IntegerDisplay node.
   *
   * @param vnid the VNID of variable or node to lookup.
   * @return determines whether vnID exists as a IntegerDisplay node. 
   *         Empty string if not.
   */
  public String checkIntegerDisplayType(Integer vnid){
    if ( m_button_by_id.containsKey(vnid) ) return "IntegerDisplayNode";
    else return "";
  }
}
