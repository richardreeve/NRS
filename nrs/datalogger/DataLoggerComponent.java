...
//TESTING!
import nrs.core.base.message.MessageToXML;
import nrs.core.base.message.XMLToMessage;
 ...
 * @author Darren Smith
 */
public class DataLoggerComponent extends BaseComponent
{
  /** Component type. */
  private final static String m_CType = "datalogger";
  /** Component version. */
  ...
  private DataLoggerNode m_rootNode;
   ...
   */
  public DataLoggerComponent(VariableManager vMan)
  {
    super(vMan,
          m_CType,
          m_SpeaksBMF,
                                         ...
                                         boolean diff) 
  {
    ...
    if ( vnType.equals("DataLoggerNode")){
      if ( m_rootNode == null ){
        m_rootNode = new DataLoggerNode(m_vMan, vnName);
        
        addRootNode(m_rootNode);

        PackageLogger.log.fine("Created a DataLoggerNode with name "
                               + vnName);
      }
      else
    ...
    // create IntegerNode
    else if ( vnType.equals("IntegerNode") ){
      
      if ( findVar(info().getCID(), vnid, vnName) == null )
        // add to root node
        m_rootNode.addNode(new IntegerNode(m_vMan, vnName));
      else
        PackageLogger.log.warning("CreateNode message received with "
                                  + vnName + " that already exists.");
    }
    // create BooleanNode
    else if ( vnType.equals("BooleanNode") ){
      
      if ( findVar(info().getCID(), vnid, vnName) == null )
        // add to root node
        m_rootNode.addNode(new BooleanNode(m_vMan, vnName));
      else
        PackageLogger.log.warning("CreateNode message received with "
                                  + vnName + " that already exists.");
    ...
    // create StringNode
    else if ( vnType.equals("StringNode") ){
      
      if ( findVar(info().getCID(), vnid, vnName) == null )
        // add to root node
        m_rootNode.addNode(new StringNode(m_vMan, vnName));
      else
        PackageLogger.log.warning("CreateNode message received with "
                                  + vnName + " that already exists.");
  ...
  public void handleMessageAt_DeleteNode(Message m,
    ...
    if ( vnid == m_rootNode.getVNID() ){
      ...
      PackageLogger.log.fine("Deleted root DataLoggerNode.");
    }
    else{
      Node n;
      //delete node
