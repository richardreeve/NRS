...
namespace
{
  NRS::Message::QueryVNNameManager sQVNNameM;
}

NRS::Message::QueryVNNameManager::QueryVNNameManager() : 
  VariableManager( Literals::Object::QUERYVNNAME_VARIABLE(), "", 4 )
{
  Base::Executive::getExecutive().addStartupVariable( getMessageName(),
						      getMessageName() );
  iDescription = "The built-in message for queries of "
    "variable or node VNNames from their VNIDs.";
}

void NRS::Message::QueryVNNameManager::doConfigure()
{
...
void
NRS::Message::QueryVNNameManager::createVariable( const std::string &aName )
  const
{
  new QueryVNName ( aName );
}
