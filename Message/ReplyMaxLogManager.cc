...
namespace
{
  NRS::Message::ReplyMaxLogManager sRMaxLogM;
}

NRS::Message::ReplyMaxLogManager::ReplyMaxLogManager() : 
  ReplyManager( Literals::Object::REPLYMAXLOG_VARIABLE(), "", 2 )
{
}

void NRS::Message::ReplyMaxLogManager::setSegments()
{

  ReplyManager::setSegments();

  iSegTypeVector[ 1 ] = NRS::Type::UnsignedManager::getName();
  iSegNameVector[ 1 ] = NRS::Literals::Attribute::LOG();
  iSegDisplayNameVector[ 1 ] = "Number of logs";
}

void NRS::Message::ReplyMaxLogManager::setDescription()
{
  iDescription = "The built-in message type for replies to requests "
    "for number of logs.";
}

void NRS::Message::ReplyMaxLogManager::createVariable( std::string aName )
{
  new ReplyMaxLog ( aName );
}

bool 
NRS::Message::ReplyMaxLogManager::deliverMessage( unsigned_t vnid,
						 std::map< std::string,
						 ...
						 const char *contents )
{
  ...
  if ( !msgHasAttribute( attrMap, Literals::Attribute::LOG() ) )
    return false;

  dynamic_cast< ReplyMaxLog* >( theVND.getVariable( vnid ))->
    receiveMessage( msgID,
		    atol( attrMap[ NRS::Literals::Attribute::LOG() ].c_str() )
		    );
...
bool 
NRS::Message::ReplyMaxLogManager::translateMessage( unsigned_t port,
						   ...
						   const char *contents )
{
  ...
  // remove attributes from attrMap
  attrMap.erase( Literals::Attribute::LOG() );
...
bool NRS::Message::ReplyMaxLogManager::deliverMessage( unsigned_t vnid,
						      char *&data,
						      bool intelligent,
						      const char* intMap[] )
{
						   ...
						   const char* intMap[] )
{
  ...
  attrMap[ Literals::Attribute::LOG() ] = anSS.str();
...
void 
NRS::Message::ReplyMaxLogManager::sendMessage( Base::Target &theTarget,
					       unsigned_t msgID,
					       unsigned_t numLogs )
{
  Base::ExternalInterfaceDirector &theEID =
