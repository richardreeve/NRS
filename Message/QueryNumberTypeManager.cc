...
namespace
{
  NRS::Message::QueryNumberTypeManager sQNumberTypeM;
}

NRS::Message::QueryNumberTypeManager::QueryNumberTypeManager() : 
  QueryManager( Literals::Object::QUERYNUMBERTYPE_VARIABLE(), "", 3 )
{
}

void NRS::Message::QueryNumberTypeManager::setSegments()
{
  QueryManager::setSegments();
}

void NRS::Message::QueryNumberTypeManager::setDescription()
{
  iDescription = "The built-in message for queries of "
    "languages capabilities";
}

void NRS::Message::QueryNumberTypeManager::createVariable( std::string aName )
{
  new QueryNumberType ( aName );
}

// tells the manager to deliver data derived from a PML message to
// QueryNumberType
bool
NRS::Message::QueryNumberTypeManager::deliverMessage( unsigned_t vnid,
						      std::map< std::string,
						      std::string >
						      &NRSAttrMap,
						      std::map< std::string,
						      ...
						      const char *contents )
{
  Base::VariableNodeDirector &theVND = Base::VariableNodeDirector::getDirector();
  ...
  dynamic_cast< QueryNumberType* >( theVND.getVariable( vnid ))->
...
bool 
NRS::Message::QueryNumberTypeManager::translateMessage( unsigned_t port,
							Base::Target
							&theTarget,
							std::map< std::string,
							std::string > 
...
bool NRS::Message::QueryNumberTypeManager::deliverMessage( unsigned_t vnid,
							   char *&data,
							   bool intelligent,
							   const char*
							   intMap[] )
...
void 
NRS::Message::QueryNumberTypeManager::sendMessage( Base::Target &theTarget,
						   Base::Target &returnTarget,
						   unsigned_t msgID )
{
  Base::ExternalInterfaceDirector &theEID =
