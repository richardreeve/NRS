...
namespace
{
  NRS::Message::QueryNumLogManager sQNumLogM;
}

NRS::Message::QueryNumLogManager::QueryNumLogManager() : 
  QueryManager( Literals::Object::QUERYNUMLOG_VARIABLE(), "", 3 )
{
}

void NRS::Message::QueryNumLogManager::setSegments()
{
  QueryManager::setSegments();
}

void NRS::Message::QueryNumLogManager::setDescription()
{
  iDescription = "The built-in message for requests for numbers of logs";
}

void NRS::Message::QueryNumLogManager::createVariable( std::string aName )
{
  new QueryNumLog ( aName );
}

// tells the manager to deliver data derived from a PML message to QueryNumLog
bool
NRS::Message::QueryNumLogManager::deliverMessage( unsigned_t vnid,
						 std::map< std::string,
						 ...
						 const char *contents )
{
  ...
  if (!extractMsgID( attrMap, msgID ))
    return false;
  dynamic_cast< QueryNumLog* >( theVND.getVariable( vnid ))->
    receiveMessage( returnTarget, msgID );
  return true;
}

bool 
NRS::Message::QueryNumLogManager::translateMessage( unsigned_t port,
...
bool NRS::Message::QueryNumLogManager::deliverMessage( unsigned_t vnid,
						      char *&data,
						      bool intelligent,
						      const char* intMap[] )
{
...
void 
NRS::Message::QueryNumLogManager::sendMessage( Base::Target &theTarget,
					       Base::Target &returnTarget,
					       unsigned_t msgID )
{
  Base::ExternalInterfaceDirector &theEID = 
