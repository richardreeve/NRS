...
namespace
{
  NRS::Message::QueryMaxPortManager sQMaxPortM;
}

NRS::Message::QueryMaxPortManager::QueryMaxPortManager() : 
  VariableManager( Literals::Object::QUERYMAXPORT_VARIABLE(), "", 3 )
{
  iDescription = "The built-in message for requests for numbers of ports";
  Base::Executive::getExecutive().addStartupVariable( getMessageName(),
						      getMessageName() );
}

void NRS::Message::QueryMaxPortManager::doConfigure()
{
...
void
NRS::Message::QueryMaxPortManager::createVariable( const std::string &aName )
  const
{
  new QueryMaxPort( aName );
}
