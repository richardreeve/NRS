...
namespace
{
  NRS::Message::QueryRouteManager sQRouteM;
}

NRS::Message::QueryRouteManager::QueryRouteManager() : 
  VariableManager( Literals::Object::QUERYROUTE_VARIABLE(), "", 4 )
{
  Base::Executive::getExecutive().addStartupVariable( getMessageName(),
						      getMessageName() );
  iDescription = "The built-in message for queries of routes";
}

void NRS::Message::QueryRouteManager::doConfigure()
{
  ...
  iSegmentDescription[3] =
    theVND.getVariableManager( Literals::Type::ROUTE() ).
    getSegmentDescription( 0 );
  iSegmentDescription[3].setAttributeName( Literals::Attribute::FORWARDROUTE(),
					   "Forward route" );
}

void
NRS::Message::QueryRouteManager::createVariable( const std::string &aName )
  const
{
  new QueryRoute ( aName );
}
