...
namespace
{
  NRS::Message::QueryVNIDManager sQVNIDM;
}

NRS::Message::QueryVNIDManager::QueryVNIDManager() : 
  VariableManager( Literals::Object::QUERYVNID_VARIABLE(), "", 4 )
{
  Base::Executive::getExecutive().addStartupVariable( getMessageName(),
						      getMessageName() );
  iDescription = "The built-in message for queries of "
    "variable or node vnids from their names.";
}

void NRS::Message::QueryVNIDManager::doConfigure()
{
  ...
  iSegmentDescription[3] =
    ...
    getSegmentDescription( 0 );
  iSegmentDescription[3].setAttributeName( Literals::Attribute::VNNAME(),
					   "VNName query" );
}

void NRS::Message::QueryVNIDManager::createVariable( const std::string &aName )
  const
{
  new QueryVNID( aName );
}
