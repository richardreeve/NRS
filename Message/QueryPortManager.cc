...
namespace
{
  NRS::Message::QueryPortManager sQPortM;
}

NRS::Message::QueryPortManager::QueryPortManager() : 
  VariableManager( Literals::Object::QUERYPORT_VARIABLE(), "", 4 )
{
  Base::Executive::getExecutive().addStartupVariable( getMessageName(),
						      getMessageName() );
  iDescription = "The built-in message for requests for ports";
}

void NRS::Message::QueryPortManager::doConfigure()
{
  ...
  iSegmentDescription[3] =
    theVND.getVariableManager( Literals::Type::INTEGER() ).
    getSegmentDescription( 0 );
  iSegmentDescription[3].setAttributeName( Literals::Attribute::PORT(),
					   "port number" );
}

void
NRS::Message::QueryPortManager::createVariable( const std::string &aName )
  const
{
  new QueryPort( aName );
}
