...
#include "ReplyMaxVNIDManager.hh"
#include "StringLiterals.hh"
#include "StringSegment.hh"
#include "Target.hh"
#include "VariableNodeDirector.hh"

NRS::Message::ReplyMaxVNID::ReplyMaxVNID( std::string aName ) :
  Variable( aName, Base::VariableNodeDirector::getDirector().
	    getVariableManager( Literals::Object::REPLYMAXVNID_VARIABLE() ) ),
  iInMsgID( dynamic_cast< Type::IntegerSegment& >( iInputLink.
						   getSegment( 0 ) ).
	    iValue ),
  iInMaxVNID( dynamic_cast< Type::IntegerSegment& >( iInputLink.
						     getSegment( 1 ) ).
	      iValue ),
  iOutMsgID( dynamic_cast< Type::IntegerSegment& >( iOutputLink.
						    getSegment( 0 ) ).
	     iValue ),
  iOutMaxVNID( dynamic_cast< Type::IntegerSegment& >( iOutputLink.
						      getSegment( 1 ) ).
	       iValue )
{
...
NRS::Message::ReplyMaxVNID::~ReplyMaxVNID()
{
}

void NRS::Message::ReplyMaxVNID::reply( Base::Target &theTarget,
					...
					integer_t maxVNID )
{
  if (theTarget.isLocal())
	...
	{
	  // add the MaxVNID to this variables messageID -> MaxVNID map
	  iMessageIDMaxVNIDMap[ aMessageID ] = maxVNID;
	  ...
	  if (!theCallbackObj)
	    _WARN_( "No callback found for MaxVNID reply\n\t" );
	  else
...
void NRS::Message::ReplyMaxVNID::doObserve( integer_t aRef )
{
  if (aRef == 0)
    {
      // get the callback director 
      ...
      if (iInMsgID != 0) 
	{
	  if ( theCD.hasCallback( iInMsgID ) )
	    {
	      // add the MaxVNID to this variables messageID -> MaxVNID map
	      iMessageIDMaxVNIDMap[ iInMsgID ] =  iInMaxVNID;
