...
#include "BooleanEnvelope.hh"
#include "BooleanEnvelopeBoolean.hh"

namespace NRS
{
  namespace Simulator
  {
    /// Factory for BooleanEnvelope nodes, which create simple
    /// envelopes of boolean output
    class BooleanEnvelopeFactory : public Base::NodeFactory
    {
    public:
      /// Default constructor
      BooleanEnvelopeFactory() : NRS::Base::NodeFactory( "BooleanEnvelope", 
							 "Boolean Envelope",
							 0, 0 )
      {
      }
      ...
      /// Empty destructor
      virtual ~BooleanEnvelopeFactory()
      {
      }
      ...
      {
	iDescription = "A boolean envelope set off by an event "
	  "after a fixed delay for a fixed length of time.";
      }
	...
	// variables for Boolean Envelope
	...
	addVariable( "Trigger", "", "Event", false, false,
		     anIDMap, aSegAttrRefMap, aSegConsMap );

	aSegAttrRefMap[ "Offset" ] = "t_offset";
	addVariable ( "Offset", "Offset to on trigger", "Time",
		      false, true, anIDMap, aSegAttrRefMap, aSegConsMap );
	aSegAttrRefMap.clear();

	aSegAttrRefMap[ "Length" ] = "t_length";
	addVariable ( "Length", "Length of envelope", "Time",
		      false, true, anIDMap, aSegAttrRefMap, aSegConsMap );
	aSegAttrRefMap.clear();
	...
	addVariable( "Activate", "", "boolean",
		     true, true, anIDMap, aSegAttrRefMap, aSegConsMap );
	...
	addAttribute( "name", "Name", "token", false, true,
		      false, true, false );
	addAttribute( "t_offset", "Offset to on trigger", "Time",
		      false, false, true, false, true, "0" );
	addAttribute( "t_length", "Length of envelope", "Time",
	...
	new BooleanEnvelope( aName, this );
	...
	new Unit::Time( aName + ".Length" );
	new Simulator::Event( aName + ".Trigger" );
	new BooleanEnvelopeBoolean ( aName + ".Activate" );
...
namespace
{
  NRS::Simulator::BooleanEnvelopeFactory sBooleanEnvelopeF;
}
