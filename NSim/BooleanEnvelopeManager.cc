...
namespace NRS
{
  namespace Simulator
  {
    /// Manager for BooleanEnvelope class, describing message structure
    class BooleanEnvelopeManager : public Base::VariableManager
    {
    public:
      /// Default constructor
      BooleanEnvelopeManager() :
	Base::VariableManager( "BooleanEnvelope",
			       "Boolean envelope",
			       2, "", false )
      {
      }
      ...
      /// Destructor
      virtual ~BooleanEnvelopeManager()
      {
      }
	...
	iSegNameVector[ 1 ] = "t_length";
      }

      /// Set Description entries for CSL output
      virtual void setDescription()
      {
	iDescription = "This variable/message type is the boolean envelope "
	  "of the simulation";
      }
	...
	if (attrMap[ iSegNameVector[1] ].find( '.' ) ==
	  ...
	  {
	    isIntNotFloat = true;
	    aTLFloat = aTLInt = atoi( attrMap[ iSegNameVector[1] ].c_str() );
	  }
	else
	  {
	    isIntNotFloat = false;
	    aTLFloat = atof( attrMap[ iSegNameVector[1] ].c_str() );
	  }

	if (isIntNotFloat)
	  dynamic_cast< BooleanEnvelope* >( theVND.getVariable( vnid ))->
	    receiveMessage( aTOInt, aTLInt );
	else
	  dynamic_cast< BooleanEnvelope* >( theVND.getVariable( vnid ))->
	    receiveMessage( aTOFloat, aTLFloat );

	// remove extracted attributes from attrMap
	attrMap.erase( "t_offset" );
	attrMap.erase( "t_length" );
	...
	*/
	...
	return true;
      ...
      /// Send a BooleanEnvelope message
      /**
       *
       * This virtual method tells the BooleanEnvelopeManager to send a
       * message
       *
       * \param theTarget the route for the message
...
namespace
{
  NRS::Simulator::BooleanEnvelopeManager sBooleanEnvelopeM;
}
