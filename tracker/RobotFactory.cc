...
#include "Robot.hh"
#include "RobotPosition.hh"

namespace NRS
{
  namespace Tracker 
  {
    class RobotFactory : public Base::NodeFactory
    {
    public:
      RobotFactory() :
        Base::NodeFactory( "Robot", "", true, true, 0, 0 )
      {}
      ...
      void setDescription()
      {
	iDescription = "A robot configuration and location.";
      }
	...
	std::map< std::string, SegmentConstraint > aSegConsMap;
        ...
        aMFOGenMap["label"] = new Base::SpecialisedMFOGenerator();
        addVariable( "RobotPosition", "Position Data", "Position", true, true,
                     anIDMap, aSegAttrRefMap, aSegConsMap, aMFOGenMap );

        aMFOGenMap.clear();
        aSegAttrRefMap[ "token" ] = "label";
	...
	addAttribute( "name", "Name", "vnname",
      ...
      {
	new Robot( nodeName, this );
        new RobotPosition( nodeName + ".RobotPosition" );
...
namespace {
  NRS::Tracker::RobotFactory sRobotF;
}
