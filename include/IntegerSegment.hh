...
namespace NRS
{
  namespace Type
  {
    /// A store for a integer_t, derived from Segment. Provides
    /// accessor methods to anonymously set and translate the
    /// underlying value into communication languages.
    class IntegerSegment : public Base::Segment
    {
    public:
      /// Default constructor
      IntegerSegment() : Segment(), iValue( 0 )
      {}

      /// Default destructor
      virtual ~IntegerSegment()
      {}
      ...
      {
	IntegerSegment *anS = new IntegerSegment();
	anS->iValue = iValue;
	return anS;
      }
...
#endif //ndef _INTEGER_SEGMENT_HH
